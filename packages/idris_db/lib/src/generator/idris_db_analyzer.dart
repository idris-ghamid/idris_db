...
class _IdrisDbAnalyzer {
  ObjectInfo analyzeCollection(Element element) {
    ...
    for (final propertyElement in modelClass.allAccessors) {
      ...
      if (!isId || property.type == IdrisDbType.string) {
        index++;
      }
    }
    ...
    return ObjectInfo(
      ...
      idrisDbName: modelClass.idrisDbName,
  ...
  void _checkValidPropertiesConstructor(
    ...
    if (properties.map((e) => e.idrisDbName).toSet().length != properties.length) {
      _err(
  ...
  PropertyInfo _analyzePropertyInfo(
    ...
    if (dartType.scalarType.element is EnumElement) {
      ...
      final enumIDRISDBType = enumProperty == null
          ? IdrisDbType.byte
          : enumProperty.type.propertyType;
      if (enumIDRISDBType != IdrisDbType.byte &&
          enumIDRISDBType != IdrisDbType.int &&
          enumIDRISDBType != IdrisDbType.long &&
          enumIDRISDBType != IdrisDbType.string) {
        _err('Unsupported enum property type.', enumProperty);
      }

      type = dartType.isDartCoreList ? enumIDRISDBType!.listType : enumIDRISDBType!;
      enumMap = {};
    ...
    final elementNullable = type.isList
        ...
        : null;
    if (isId) {
      if (type != IdrisDbType.long && type != IdrisDbType.string) {
    ...
    if ((type == IdrisDbType.byte && nullable) ||
        (type == IdrisDbType.byteList && (elementNullable ?? false))) {
    ...
    return PropertyInfo(
      ...
      idrisDbName: property.idrisDbName,
      typeClassName: type == IdrisDbType.json
          ? dartType.element!.name!
          : dartType.scalarType.element!.name!,
      targetidrisDbName: type.isObject
          ? dartType.scalarType.element!.idrisDbName
          : null,
      type: type,
      isId: isId,
      enumMap: enumMap,
  ...
  String _defaultValue(DartType type) {
    if (type.nullabilitySuffix == NullabilitySuffix.question ||
      ...
      if (type.propertyType == IdrisDbType.byte) {
        return '0';
      } else if (type.propertyType == IdrisDbType.int) {
        return '$_nullInt';
      } else {
        return '$_nullLong';
      }
  ...
  Iterable<IndexInfo> analyzeObjectIndex(
    ...
    for (final index in element.indexAnnotations) {
      final indexProperties = [element.idrisDbName, ...index.composite];
      ...
      for (var i = 0; i < indexProperties.length; i++) {
        ...
        final property = properties
            .where((it) => it.idrisDbName == propertyName)
            .firstOrNull;
        if (property == null) {
        ...
        } else if (property.type == IdrisDbType.json) {
      ...
      _checkidrisDbName(name, element);
