...
extension QueryAsync<T> on IdrisDbQuery<T> {
  /// {@macro query_find_first}
  Future<T?> findFirstAsync({int? offset}) =>
      idrisDb.readAsync((idb) => findFirst(offset: offset));

  /// {@macro query_find_all}
  Future<List<T>> findAllAsync({int? offset, int? limit}) =>
      idrisDb.readAsync((idb) => findAll(offset: offset, limit: limit));

  /// {@macro aggregation_count}
  Future<int> countAsync() => idrisDb.readAsync((idb) => count());

  /// {@macro aggregation_is_empty}
  Future<bool> isEmptyAsync() => idrisDb.readAsync((idb) => isEmpty());

  /// @nodoc
  @protected
  Future<R?> aggregateAsync<R>(Aggregation op) =>
      idrisDb.readAsync((idb) => aggregate(op));
}

/// Aggregation operations for number queries.
extension QueryNumAggregation<T extends num?> on IdrisDbQuery<T?> {
...
extension QueryDateAggregation<T extends DateTime?> on IdrisDbQuery<T> {
  /// {@macro aggregation_min}
...
extension QueryStringAggregation<T extends String?> on IdrisDbQuery<T?> {
  /// {@macro aggregation_min}
  T? min() => aggregate(Aggregation.min);
...
extension QueryExecute<OBJ, R> on QueryBuilder<OBJ, R, QOperations> {
  ...
  IdrisDbQuery<R> build() => _query.build();

  T _withQuery<T>(T Function(IdrisDbQuery<R> q) f) {
    final q = build();
    try {
      return f(q);
    } finally {
  ...
  Future<T> _withQueryAsync<T>(Future<T> Function(IdrisDbQuery<R> q) f) async {
    final q = build();
    try {
      return await f(q);
    } finally {
