...
extension on int {
  ...
  Never throwError() {
    switch (this) {
      ...
      default:
        final length = IdrisDbCore.b.IDRISDB_get_error(IdrisDbCore.stringPtrPtr);
        final ptr = IdrisDbCore.stringPtr;
        if (length != 0 && !ptr.isNull) {
          final error = utf8.decode(ptr.asU8List(length));
          throw DatabaseError(error);
        } else {
          throw DatabaseError(
            'There was an error but it could not be loaded from IdrisDbCore.',
          );
        }
    }
  }
