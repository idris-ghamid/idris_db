...
use crate::core::query_builder::{IDRISDBQueryBuilder, Sort};
...
impl<'a> IDRISDBQueryBuilder for NativeQueryBuilder<'a> {
...
fn condition_to_native(
    ...
    let filter = match condition.condition_type {
        ...
        ConditionType::StringStartsWith => {
            ...
            let upper = format!("{}{}", lower, IDRISDBValue::MAX_STRING);
            NativeFilter::string(
                property?,
                Some(lower),
                Some(&upper),
...
fn lower_real(value: Option<&IDRISDBValue>, include: bool) -> Option<f64> {
...
fn upper_real(value: Option<&IDRISDBValue>, include: bool) -> Option<f64> {
    let mut value = if let Some(value) = value {
        value.real()?
    } else {
        f64::NAN
...
fn get_max(property: Option<&NativeProperty>) -> Option<IDRISDBValue> {
    let value = if let Some(property) = property {
        match property.data_type {
            DataType::Bool | DataType::BoolList => IDRISDBValue::Bool(true),
            DataType::Byte | DataType::ByteList => IDRISDBValue::Integer(u8::MAX as i64),
            DataType::Int | DataType::IntList => IDRISDBValue::Integer(i32::MAX as i64),
            DataType::Float | DataType::FloatList => IDRISDBValue::Real(f64::INFINITY),
            DataType::Long | DataType::LongList => IDRISDBValue::Integer(i64::MAX),
            DataType::Double | DataType::DoubleList => IDRISDBValue::Real(f64::INFINITY),
            DataType::String | DataType::StringList | DataType::Json => {
                IDRISDBValue::String(IDRISDBValue::MAX_STRING.to_string())
            }
            DataType::Object | DataType::ObjectList => return None,
        }
    } else {
        IDRISDBValue::Integer(i64::MAX)
    };
    Some(value)
}
