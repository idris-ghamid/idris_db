...
use super::native_open::{get_IDRISDB_path, open_native};
...
impl NativeInstance {
    ...
    pub(crate) fn verify_instance_id(&self, instance_id: u32) -> Result<()> {
        if self.instance_id != instance_id {
            Err(IdrisDbError::InstanceMismatch {})
        } else {
            Ok(())
        }
    }
...
impl IDRISDBInstance for NativeInstance {
    ...
    fn close(instance: Arc<Self>, delete: bool) -> bool {
        ...
        if Arc::strong_count(&instance) == 2 {
            ...
            if Arc::strong_count(&instance) == 2 {
                ...
                if delete {
                    let mut path = get_IDRISDB_path(&instance.name, &instance.dir);
                    drop(instance);
