...
impl NativeIndex {
    ...
    fn create_key(&self, object: IDRISDBDeserializer, buffer: Vec<u8>) -> (Vec<u8>, bool) {
    ...
    /* pub fn iter_between<'txn, 'env>(
        &self,
        cursors: &IDRISDBCursors<'txn, 'env>,
        lower_key: &IndexKey,
        upper_key: &IndexKey,
    ...
    pub fn get_size(&self, cursors: &IDRISDBCursors) -> Result<u64> {
        Ok(cursors.db_stat(self.db)?.1)
    }

    pub fn clear(&self, cursors: &IDRISDBCursors) -> Result<()> {
        cursors.clear_db(self.db)
    }

    pub fn verify(&self, cursors: &IDRISDBCursors, objects: &IntMap<IDRISDBObject>) -> Result<()> {
        let mut count = 0;
