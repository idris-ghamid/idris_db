...
pub(crate) fn mdbx_result(err_code: c_int) -> Result<()> {
    match err_code {
        ...
        mdbx_sys::MDBX_MAP_FULL => Err(IdrisDbError::DbFull {}),
        other => Err(mdbx_error(other)),
    }
}

#[inline]
pub(crate) fn mdbx_error(err_code: c_int) -> IdrisDbError {
    match err_code {
        mdbx_sys::MDBX_MAP_FULL => IdrisDbError::DbFull {},
        other => unsafe {
            let err_raw = mdbx_sys::mdbx_strerror(other);
            let err = CStr::from_ptr(err_raw);
            IdrisDbError::DbError {
                code: other,
                message: err
                    .to_str()
...
pub(crate) mod osal {
    ...
    pub fn str_to_os(str: &str) -> Result<U16CString> {
        U16CString::from_str(str).map_err(|_| IdrisDbError::IllegalString {})
    }
...
pub(crate) mod osal {
    ...
    pub fn str_to_os(str: &str) -> Result<CString> {
        CString::new(str.as_bytes()).map_err(|_| IdrisDbError::IllegalString {})
    }
