...
impl<'a> IDRISDBReader for NativeReader<'a> {
...
pub struct NativeListReader<'a> {
    list: IDRISDBDeserializer<'a>,
    data_type: DataType,
    embedded_collection_index: Option<u16>,
    all_collections: &'a [NativeCollection],
}

impl<'a> IDRISDBReader for NativeListReader<'a> {
    type ObjectReader<'b> = NativeReader<'b> where 'a: 'b;
...
mod test {
    ...
    fn test_reader_id_name() {
        ...
        let reader = NativeReader::new(
            0,
            IDRISDBDeserializer::from_bytes(&[0, 0, 0]),
            &collection,
            &[],
        );
    ...
    fn test_reader_read_id() {
        ...
        let bytes = [0, 0, 0];
        let reader = NativeReader::new(55, IDRISDBDeserializer::from_bytes(&bytes), &col, &[]);
    ...
    fn test_reader_read_bool() {
        ...
        let reader = NativeReader::new(0, IDRISDBDeserializer::from_bytes(&bytes), &col, &[]);
    ...
    fn test_reader_read_object() {
        ...
        let cols = [col2];
        let reader = NativeReader::new(0, IDRISDBDeserializer::from_bytes(&bytes), &col1, &cols);
