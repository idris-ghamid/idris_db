...
pub(crate) struct IDRISDBDeserializer<'a> {
    pub bytes: &'a [u8],
    static_size: u32,
}

impl<'a> IDRISDBDeserializer<'a> {
    #[inline]
    ...
    #[inline]
    pub fn read_nested(&self, offset: u32) -> Option<IDRISDBDeserializer<'a>> {
        let offset = self.get_offset(offset)?;
    ...
    ) -> Option<(IDRISDBDeserializer<'a>, u32)> {
...
mod test {
    ...
    fn test_read_bool_contains_offset() {
        ...
        let deserializer = IDRISDBDeserializer::from_bytes(&bytes);
