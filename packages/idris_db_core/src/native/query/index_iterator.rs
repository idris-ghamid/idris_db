...
impl<'a> Iterator for IndexIterator<'a> {
    type Item = (i64, IDRISDBDeserializer<'a>);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some((key, value)) = next {
            if let Some(primary_cursor) = &mut self.primary_cursor {
                ...
                return Some((id.to_id(), IDRISDBDeserializer::from_bytes(object)));
            } else {
                return Some((key.to_id(), IDRISDBDeserializer::from_bytes(value)));
            }
        } else {
