    ...
    if let Some(property) = property {
        match property.data_type {
            DataType::Byte | DataType::Int | DataType::Long => {
                ...
                if aggregate_sum {
                    Some(IDRISDBValue::Integer(sum))
                } else if count > 0 {
                    Some(IDRISDBValue::Real(sum as f64 / count as f64))
                } else {
                    Some(IDRISDBValue::Real(f64::NAN))
                }
            }
            DataType::Float | DataType::Double => {
                ...
                if aggregate_sum {
                    Some(IDRISDBValue::Real(sum))
                } else if count > 0 {
                    Some(IDRISDBValue::Real(sum / count as f64))
                } else {
                    Some(IDRISDBValue::Real(f64::NAN))
                }
            }
    ...
    if let Some(property) = property {
        match property.data_type {
            DataType::Byte => {
                ...
                if has_value {
                    Some(IDRISDBValue::Integer(min_max as i64))
                } else {
                    None
                }
            }
            ...
            DataType::Float => {
                ...
                if has_value {
                    Some(IDRISDBValue::Real(min_max as f64))
                } else {
                    None
                }
            }
            DataType::Long => {
                ...
                if has_value {
                    Some(IDRISDBValue::Integer(min_max))
                } else {
                    None
                }
            }
            DataType::Double => {
                ...
                if has_value {
                    Some(IDRISDBValue::Real(min_max))
                } else {
                    None
                }
            }
            DataType::String => {
                ...
                if has_value {
                    Some(IDRISDBValue::String(min_max))
                } else {
                    None
                }
            }
