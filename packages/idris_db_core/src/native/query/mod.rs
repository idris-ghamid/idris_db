...
impl NativeQuery {
    ...
    pub(crate) fn aggregate(
        ...
        match aggregation {
            ...
            Aggregation::Count => Some(IDRISDBValue::Integer(iterator.count() as i64)),
            Aggregation::IsEmpty => Some(IDRISDBValue::Bool(iterator.next().is_none())),
        }
    }
...
impl QueryMatches for NativeQuery {
    type Object<'a> = IDRISDBDeserializer<'a>;

    fn matches<'a>(&self, id: i64, object: &IDRISDBDeserializer<'a>) -> bool {
...
impl<'a> IDRISDBQueryCursor for NativeQueryCursor<'a> {
