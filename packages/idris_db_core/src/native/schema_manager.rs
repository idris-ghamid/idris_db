...
use crate::core::schema::{IdrisDbSchema, PropertySchema};
use itertools::Itertools;
use std::borrow::Cow;
use std::sync::Arc;

const IDRISDB_FILE_VERSION: u8 = 3;

pub(crate) fn perform_migration(
    instance_id: u32,
    env: &Arc<Env>,
    mut schemas: Vec<IdrisDbSchema>,
    ...
    for schema in schemas.iter_mut() {
        ...
        schema.version = IDRISDB_FILE_VERSION;
...
fn get_schemas(txn: &NativeTxn, info_db: Db) -> Result<Vec<IdrisDbSchema>> {
    let info_cursor = txn.get_cursor(info_db)?;
    let mut schemas = vec![];
    for (_, bytes) in info_cursor.iter()? {
        let col =
            serde_json::from_slice::<IdrisDbSchema>(bytes).map_err(|_| IdrisDbError::SchemaError {
...
fn save_schema(txn: &NativeTxn, info_db: Db, schema: &IdrisDbSchema) -> Result<()> {
    let mut info_cursor = txn.get_cursor(info_db)?;
    let bytes = serde_json::to_vec(schema).map_err(|_| IdrisDbError::SchemaError {
...
fn delete_collection(txn: &NativeTxn, info_db: Db, schema: &IdrisDbSchema) -> Result<()> {
...
fn migrate_collection(
    ...
    existing_schema: &IdrisDbSchema,
) -> Result<Vec<PropertySchema>> {
    if existing_schema.version != IDRISDB_FILE_VERSION {
        return Err(IdrisDbError::VersionError {});
    }
