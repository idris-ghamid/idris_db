...
use crate::core::writer::IDRISDBWriter;
...
pub(crate) trait WriterImpl<'a> {
    ...
    fn get_serializer(&mut self) -> &mut IDRISDBSerializer;
}

impl<'a, T: WriterImpl<'a>> IDRISDBWriter<'a> for T {
...
mod tests {
    ...
    pub fn test_write_null() {
        ...
        bytes.extend_from_slice(&IDRISDBObject::NULL_INT.to_le_bytes());
        assert_eq!(b.finish().as_bytes(), &bytes);

        builder!(b, p, Float);
        b.write_null(p.offset, p.data_type);
        let mut bytes = vec![7, 0, 255];
        bytes.extend_from_slice(&IDRISDBObject::NULL_FLOAT.to_le_bytes());
        assert_eq!(b.finish().as_bytes(), &bytes);

        builder!(b, p, Long);
        b.write_null(p.offset, p.data_type);
        let mut bytes = vec![11, 0, 255];
        bytes.extend_from_slice(&IDRISDBObject::NULL_LONG.to_le_bytes());
        assert_eq!(b.finish().as_bytes(), &bytes);

        builder!(b, p, Double);
        b.write_null(p.offset, p.data_type);
        let mut bytes = vec![11, 0, 255];
        bytes.extend_from_slice(&IDRISDBObject::NULL_DOUBLE.to_le_bytes());
        assert_eq!(b.finish().as_bytes(), &bytes);
    ...
    pub fn test_write_bool() {
        ...
        assert_eq!(b.finish().as_bytes(), &[4, 0, 255, IDRISDBObject::TRUE_BOOL]);

        builder!(b, p, Bool);
        b.write_bool(p.offset, Some(false));
        assert_eq!(b.finish().as_bytes(), &[4, 0, 255, IDRISDBObject::FALSE_BOOL]);

        builder!(b, p, Bool);
        b.write_bool(p.offset, None);
        assert_eq!(b.finish().as_bytes(), &[4, 0, 255, IDRISDBObject::NULL_BOOL]);
    }
    ...
    pub fn test_write_object() {
        builder!(b, p, Object);
        let object = IDRISDBObject::from_bytes(&[3, 0, 111]);
    ...
    pub fn test_write_object_list() {
        ...
        let object1 = IDRISDBObject::from_bytes(&[2, 0]);
        let object2 = IDRISDBObject::from_bytes(&[3, 0, 123]);
