...
pub struct NativeInsert<'a> {
    ...
    remaining: u32,
    pub(crate) object: IDRISDBSerializer,
}
...
impl<'a> NativeInsert<'a> {
    pub(crate) fn new(
        ...
        let insert = Self {
            ...
            all_collections,
            remaining: count,
            object: IDRISDBSerializer::new(buffer, 0, collection.static_size),
        };
        Ok(insert)
    }
}

impl<'a> IDRISDBInsert<'a> for NativeInsert<'a> {
    type Txn = NativeTxn;
    ...
    fn save(&mut self, id: i64) -> Result<()> {
        if self.remaining > 0 {
            ...
            if buffer.len() > MAX_OBJ_SIZE as usize {
                return Result::Err(IdrisDbError::ObjectLimitReached {});
            }
            self.txn_cursor.put(self.collection, id, &buffer, self.all_collections)?;

            self.remaining -= 1;
            buffer.clear();
            self.object = IDRISDBSerializer::new(buffer, 0, self.collection.static_size);
            Ok(())
        } else {
    ...
    fn finish(self) -> Result<Self::Txn> {
        if self.remaining > 0 {
            Err(IdrisDbError::InsertIncomplete {})
        } else {
