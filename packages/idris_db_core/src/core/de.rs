...
use super::error::IdrisDbError;
use super::insert::IDRISDBInsert;
use super::instance::IDRISDBInstance;
...
pub(super) struct IDRISDBJsonImportVisitor<'a, I: IDRISDBInstance, F: Fn(&str) -> i64> {
    instance: &'a I,
...
impl<'a, I: IDRISDBInstance, F: Fn(&str) -> i64> IDRISDBJsonImportVisitor<'a, I, F> {
    pub(super) fn new(instance: &'a I, txn: I::Txn, collection_index: u16, str_to_id: F) -> Self {
        IDRISDBJsonImportVisitor {
            instance,
    ...
    fn import(&self, objects: &[&str]) -> Result<(), IdrisDbError> {
        ...
        for object in objects {
            ...
            let visitor = IDRISDBObjectVisitor::new(insert, &self.str_to_id);
            let (id, writer) =
                deser
            ...
            if let Some(id) = id {
                ...
                return Err(IdrisDbError::JsonError {
...
impl<'a, 'de, I: IDRISDBInstance, F: Fn(&str) -> i64> Visitor<'de>
    for IDRISDBJsonImportVisitor<'a, I, F>
{
...
struct IDRISDBObjectVisitor<'a, W: IDRISDBWriter<'a>, F: Fn(&str) -> i64> {
    writer: W,
    str_to_id: &'a F,
}

impl<'a, W: IDRISDBWriter<'a>, F: Fn(&str) -> i64> IDRISDBObjectVisitor<'a, W, F> {
    pub fn new(writer: W, str_to_id: &'a F) -> Self {
        IDRISDBObjectVisitor { writer, str_to_id }
    }
}
...
impl<'a, 'de: 'a, W: IDRISDBWriter<'a>, F: Fn(&str) -> i64> Visitor<'de>
    for IDRISDBObjectVisitor<'a, W, F>
{
                    ...
                    DataType::Object => {
                        ...
                        if let Some(value) = value {
                            if let Some(object_writer) = self.writer.begin_object(index as u32) {
                                ...
                                let visitor = IDRISDBObjectVisitor::new(object_writer, self.str_to_id);
                                let (_, object) = deser
                                    .deserialize_map(visitor)
                                ...
                                self.writer.begin_list(index as u32, list.len() as u32)
                            {
                                for (i, value) in list.iter().enumerate() {
                                    if let Some(value) = value {
                                        ...
                                        let visitor =
                                            IDRISDBObjectVisitor::new(object, self.str_to_id);
                                        let (_, object) = deser
                                            .deserialize_map(visitor)
                                            .map_err(|e| Error::custom(e))?;
