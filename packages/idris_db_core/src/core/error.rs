...
pub type Result<T> = std::result::Result<T, IdrisDbError>;

#[derive(Clone, Debug, Snafu, Eq, PartialEq)]
#[repr(u8)]
pub enum IdrisDbError {
    #[snafu(display(
    ...
    #[snafu(display("IdrisDb version of the file is too new or too old to be used."))]
    VersionError {},

    #[snafu(display(
        "Operation not supported. You might need to use a different flavor of IdrisDb."
    ))]
