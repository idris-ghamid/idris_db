...
use super::{data_type::DataType, ser::IDRISDBObjectSerialize};
use serde::Serializer;
use std::borrow::Cow;

pub trait IDRISDBReader {
    type ObjectReader<'b>: IDRISDBReader
    where
        Self: 'b;

    type ListReader<'b>: IDRISDBReader
    where
        Self: 'b;
    ...
    where
        Self: Sized,
    {
        let obj = IDRISDBObjectSerialize::new(self);
        serializer
            .serialize_some(&obj)
