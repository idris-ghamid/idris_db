...
fn equal(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
        (Value::Null, None) => true,
        (Value::Bool(value), Some(IDRISDBValue::Bool(cond_value))) => value == cond_value,
        (Value::Number(value), Some(IDRISDBValue::Integer(cond_value))) => {
            value.as_i64() == Some(*cond_value)
        }
        (Value::Number(value), Some(IDRISDBValue::Real(cond_value))) => {
            value.as_f64() == Some(*cond_value)
        }
        (Value::String(value), Some(IDRISDBValue::String(cond_value))) => {
            if case_sensitive {
                value == cond_value
            } else {
...
fn greater(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
...
fn greater_or_equal(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
        (_, None) => true,
        (Value::Bool(value), Some(IDRISDBValue::Bool(cond_value))) => value >= cond_value,
        (Value::Number(value), Some(IDRISDBValue::Integer(cond_value))) => {
...
fn less(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
...
fn less_or_equal(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
        (Value::Null, _) => true,
        (Value::Bool(value), Some(IDRISDBValue::Bool(cond_value))) => value <= cond_value,
        (Value::Number(value), Some(IDRISDBValue::Integer(cond_value))) => {
...
fn between(
    value: &Value,
    lower: &Option<IDRISDBValue>,
    upper: &Option<IDRISDBValue>,
    case_sensitive: bool,
) -> bool {
    match (value, lower, upper) {
        (value, None, upper) => less_or_equal(value, upper, case_sensitive),
        (Value::Bool(value), Some(IDRISDBValue::Bool(lower)), Some(IDRISDBValue::Bool(upper))) => {
            value >= lower && value <= upper
        }
        (
            Value::Number(value),
            Some(IDRISDBValue::Integer(lower)),
            Some(IDRISDBValue::Integer(upper)),
        ) => value.as_i64() >= Some(*lower) && value.as_i64() <= Some(*upper),
        (Value::Number(value), Some(IDRISDBValue::Real(lower)), Some(IDRISDBValue::Real(upper))) => {
            value.as_f64() >= Some(*lower) && value.as_f64() <= Some(*upper)
        }
        (Value::String(value), Some(IDRISDBValue::String(lower)), Some(IDRISDBValue::String(upper))) => {
            if case_sensitive {
                value >= lower && value <= upper
            } else {
...
fn string_starts_with(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
        (Value::String(value), Some(IDRISDBValue::String(cond_value))) => {
            if case_sensitive {
...
fn string_ends_with(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
        (Value::String(value), Some(IDRISDBValue::String(cond_value))) => {
            if case_sensitive {
...
fn string_contains(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
        (Value::String(value), Some(IDRISDBValue::String(cond_value))) => {
            if case_sensitive {
...
fn string_matches(value: &Value, cond_value: &Option<IDRISDBValue>, case_sensitive: bool) -> bool {
    match (value, cond_value) {
        (Value::String(value), Some(IDRISDBValue::String(cond_value))) => {
            if case_sensitive {
