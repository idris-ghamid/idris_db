...
use super::{data_type::DataType, error::IdrisDbError};
use itertools::Itertools;
use serde::{Deserialize, Serialize};

#[derive(Serialize, Deserialize, Clone, Hash, Debug, PartialEq, Eq)]
pub struct IdrisDbSchema {
    pub name: String,
...
impl IdrisDbSchema {
    ...
    pub fn from_json(json: &[u8]) -> Result<Vec<Self>> {
        if let Ok(collections) = serde_json::from_slice::<Vec<IdrisDbSchema>>(json) {
            Ok(collections)
        } else {
    ...
    fn verify(&self, collections: &[IdrisDbSchema]) -> Result<()> {
...
mod test {
    ...
    fn test_verify_checks_name() {
        let schema = IdrisDbSchema::new("test", None, Vec::new(), Vec::new(), false);
        assert!(schema.verify(&[]).is_ok());

        let schema = IdrisDbSchema::new("_test", None, Vec::new(), Vec::new(), false);
        assert!(schema.verify(&[]).is_err());

        let schema = IdrisDbSchema::new("sqlite_test", None, Vec::new(), Vec::new(), false);
        assert!(schema.verify(&[]).is_err());

        let schema = IdrisDbSchema::new("", None, Vec::new(), Vec::new(), false);
        assert!(schema.verify(&[]).is_err());
    }

    #[test]
    fn test_verify_checks_index_names() {
        let schema = IdrisDbSchema::new(
            "test",
            None,
            Vec::new(),
    ...
    fn test_verify_checks_non_object_property_has_no_target() {
        ...
        let schema2 = IdrisDbSchema::new("test2", None, vec![], Vec::new(), false);
    ...
    fn test_verify_checks_index_properties_are_not_object() {
        ...
        let schema2 = IdrisDbSchema::new("test2", None, vec![], vec![], true);
