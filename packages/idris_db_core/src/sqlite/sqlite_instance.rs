...
use crate::core::instance::{Aggregation, CompactCondition, IDRISDBInstance};
use crate::core::query_builder::IDRISDBQueryBuilder;
use crate::core::schema::IdrisDbSchema;
...
impl SQLiteInstance {
    fn get_collection(&self, collection_index: u16) -> Result<&SQLiteCollection> {
        if let Some(collection) = self.info.collections.get(collection_index as usize) {
            ...
            Err(IdrisDbError::IllegalArgument {})
        }
    }
}

impl IDRISDBInstance for SQLiteInstance {
    type Instance = Self;
    ...
    fn begin_txn(&self, write: bool) -> Result<SQLiteTxn> {
        ...
        if self.txn_active.replace(true) {
            Err(IdrisDbError::TransactionActive {})
        } else {
    ...
    fn update(
        ...
        qb.set_filter(Filter::Condition(FilterCondition::new(
            0,
            ConditionType::Equal,
            vec![Some(IDRISDBValue::Integer(id))],
            false,
        )));
        let q = qb.build();
    ...
    fn count(&self, txn: &Self::Txn, collection_index: u16) -> Result<u32> {
        ...
        if let Some(IDRISDBValue::Integer(count)) = result {
            Ok(count as u32)
        } else {
            Ok(0)
        }
