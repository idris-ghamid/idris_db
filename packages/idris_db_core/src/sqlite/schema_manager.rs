...
pub(crate) fn perform_migration(txn: &SQLiteTxn, schemas: &[IdrisDbSchema]) -> Result<()> {
    txn.guard(|| {
...
fn read_col_schema(sqlite: &SQLite3, name: &str) -> Result<IdrisDbSchema> {
    ...
    if let Some(id_prop_index) = id_prop_index {
        ...
        return Err(IdrisDbError::SchemaError {
    ...
    Ok(IdrisDbSchema::new(name, None, properties, indexes, false))
}

fn update_table(sqlite: &SQLite3, collection: &IdrisDbSchema) -> Result<()> {
