...
use crate::core::schema::{IndexSchema, IdrisDbSchema, PropertySchema};
...
pub(crate) fn create_table_sql(collection: &IdrisDbSchema) -> String {
    format!(
...
pub(crate) fn add_column_sql(collection: &IdrisDbSchema, property: &PropertySchema) -> String {
    format!(
...
pub(crate) fn drop_column_sql(collection: &IdrisDbSchema, property_name: &str) -> String {
    format!(
...
fn condition_sql(
    ...
    let sql = match condition.condition_type {
        ...
        ConditionType::StringStartsWith => {
            if let Some(IDRISDBValue::String(prefix)) = condition.values.get(0)? {
                values.push(IDRISDBValue::String(format!("{}%", escape_wildcard(prefix))));
        ...
        ConditionType::StringEndsWith => {
            if let Some(IDRISDBValue::String(postfix)) = condition.values.get(0)? {
                values.push(IDRISDBValue::String(format!("%{}", escape_wildcard(postfix))));
                format!("{} LIKE ? ESCAPE '\\'", property_name)
            } else {
                "FALSE".to_string()
            }
        ...
        ConditionType::StringContains => {
            if let Some(IDRISDBValue::String(needle)) = condition.values.get(0)? {
                values.push(IDRISDBValue::String(format!("%{}%", escape_wildcard(needle))));
                format!("{} LIKE ? ESCAPE '\\'", property_name)
            } else {
                "FALSE".to_string()
            }
        ...
        ConditionType::StringMatches => {
            if let Some(IDRISDBValue::String(wildcard)) = condition.values.get(0)? {
                let wildcard = escape_wildcard(wildcard)
                    .replace("*", "%")
                    .replace("?", "_");
                values.push(IDRISDBValue::String(wildcard));
                format!("{} LIKE ? ESCAPE '\\'", property_name)
            } else {
                "FALSE".to_string()
            }
...
pub(crate) const FN_FILTER_JSON_NAME: &str = "IDRISDB_filter_json";
