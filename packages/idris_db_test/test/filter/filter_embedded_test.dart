...
void main() {
  group('Filter embedded', () {
    ...
    setUp(() async {
      ...
      IdrisDb.write(
        (IdrisDb) => IdrisDb.models.putAll([obj1, obj2, obj3, obj4, obj5, obj6]),
      );
    });

    IdrisDbTest('.embedded()', () {
      expect(
        IdrisDb.models
            .where()
    ...
    IdrisDbTest('.embeddedIsNull()', () {
      expect(IdrisDb.models.where().embeddedBIsNull().findAll(), [obj2, obj6]);
    });

    IdrisDbTest('.embeddedIsNotNull()', () {
      expect(IdrisDb.models.where().embeddedBIsNotNull().findAll(), [
        obj1,
        obj3,
        obj4,
        obj5,
    ...
    IdrisDbTest('.embedded() then .embedded()', () {
      expect(
        IdrisDb.models
            .where()
    ...
    IdrisDbTest('.embedded() then .embeddedIsNull()', () {
      expect(
        IdrisDb.models.where().embeddedA((q) => q.embeddedBIsNull()).findAll(),
        [obj3, obj5],
      );
    });

    IdrisDbTest('.embedded() then .embeddedIsNotNull()', () {
      expect(
        IdrisDb.models.where().embeddedA((q) => q.embeddedBIsNotNull()).findAll(),
        [obj1, obj2, obj4, obj6],
      );
    });
  });
