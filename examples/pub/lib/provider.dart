...
final IDRISDBPod = FutureProvider((ref) async {
  final dir = await getApplicationDocumentsDirectory();
  return IdrisDb.open(schemas: [PackageSchema, AssetSchema], directory: dir.path);
});
...
final packageManagerPod = FutureProvider((ref) async {
  final IdrisDb = await ref.watch(IDRISDBPod.future);
  final repository = ref.watch(repositoryPod);
  return PackageManager(IdrisDb, repository);
});
